...
class CircularList {
	...
	{	
		...
		if(head == null) {//리스트가 비었을 경우
			head = newNode;
			head.link = head;//자기자신을 가리키도록 설정
		}else if(cc.compare(element, head.data) <= 0) {//새 노드가 헤드보다 작거나 같을 경우
			Node3 p = head;
			while(p != head) {//마지막 노드까지 이동
				p = p.link;
			}
			p.link = newNode;//마지막 노드의 다음을 새 노드로 연결
			newNode.link = head;//새 노드의 다음을 head 로 설정(원형)
			head = newNode;//헤드 갱신
		}
		else {// 새 노드가 헤드보다 큰 경우
			Node3 current = head;
			//클 경우
			while(current.link != head && cc.compare(element, current.link.data)>0) {
				current = current.link;
			}
			newNode.link = current.link;//새 노드의 다음을 현재 노드의 다음으로 설정
			current.link = newNode;//현재 노드의 다음을 새 노드로 설정
		}
