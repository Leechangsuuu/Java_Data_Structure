...
class Graph {
	...
	// Driver-main에서 호출되는 함수를 driver라고 한다
	void DFS(int v) {
		for (int i = 0; i < n; i++)
			visited[i] = false; // initially, no vertices have been visited
	...
	// Workhorse
	void _DFS(int v)
	// visit all previously unvisited vertices that are reachable from vertex v=0 v는 출발노드
	{
		visited[v] = true;
		//구현 = c++로 구현된 알고리즘을 사용하여 자바코드로
	}
	// Workhorse 2
