...
class LinkedList2 {
	...
	//전달된 element를 찾을 때 comparator 객체를 사용한다 
	{
		...
		if(cc.compare(first.data,element)==0) {//첫번쨰 데이터가 삭제할 값이면 다음에 있는 값을 first로 설정
			first = first.link;
			return 1; // 삭제했으니까 1반환
		}
		
		Node2 q = first;//포인터 q 초기화
		
		while(q.link != null) {//첫번쨰 노드 제외한 나머지 노드 순회
			if(cc.compare(q.link.data , element)== 0) {//값 찾으면 
				q.link = q.link.link;//연결끊고 다음에 있던 노드랑 연결
				return 1;
			}
			q = q.link;//q를 다음 노드로 이동
		}
		return -1;//순회 다 했는데 값 못찾으면 -1반환
	}
	...
	//임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		Node2 p = first , q = null;//p는 현재노드 , q는 이전노드
		while(p != null) {
			if(p != null && cc.compare(element, p.data) > 0) {
				q = p;//
				p = p.link;
			}
		}
